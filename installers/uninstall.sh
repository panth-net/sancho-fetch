...
#   - the `sancho` CLI (via `uv tool uninstall sancho-fetch` and legacy `uv tool uninstall sancho`)
#   - the library pointer at ~/.sancho/
#   - AI assistant skills at ~/.claude/skills/sancho{,-update}/ and ~/.agents/skills/sancho{,-update}/
...
if command -v uv >/dev/null 2>&1; then
  removed_uv_tool=false
  for tool_name in sancho-fetch sancho; do
    if uv tool list 2>/dev/null | grep -q "^$tool_name"; then
      if uv tool uninstall "$tool_name" >/dev/null 2>&1; then
        ok "Removed sancho CLI (uv tool uninstall $tool_name)"
        removed_uv_tool=true
      else
        warn "uv tool uninstall $tool_name failed; remove it manually with that command"
      fi
    fi
  done
  if ! $removed_uv_tool; then
...
#    macOS uses ~/Library/Application Support/Claude/. The Linux path is only
#    checked for cleanup (setup never writes it; a user may have added the
#    snippet manually on a community Linux build).
remove_claude_entry() {
  local config_path="$1"
