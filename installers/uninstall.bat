...
REM   - the `sancho` CLI (via `uv tool uninstall sancho-fetch` and legacy `uv tool uninstall sancho`)
...
if errorlevel 1 (
  ...
  set "REMOVED_UV_TOOL=0"
  for %%T in (sancho-fetch sancho) do (
    uv tool list 2>NUL | findstr /b /c:"%%T" >NUL
    if not errorlevel 1 (
      uv tool uninstall %%T >NUL 2>&1
      if errorlevel 1 (
        echo   !!  uv tool uninstall %%T failed; remove it manually with that command
      ) else (
        echo   OK  Removed sancho CLI ^(uv tool uninstall %%T^)
        set "REMOVED_UV_TOOL=1"
      )
    )
  )
  if "!REMOVED_UV_TOOL!"=="0" (
...
REM Exit codes: 0 = removed, 2 = no entry, 1 = crash (unreadable/corrupt JSON).
%~1 -c "import json,sys,pathlib; p=pathlib.Path(sys.argv[1]); d=json.loads(p.read_text(encoding='utf-8')); s=d.get('mcpServers') if isinstance(d,dict) else None; sys.exit(2) if not isinstance(s,dict) or 'sancho' not in s else (s.pop('sancho'), p.write_text(json.dumps(d,indent=2)+'\n',encoding='utf-8'), sys.exit(0))" %2
if errorlevel 2 (
