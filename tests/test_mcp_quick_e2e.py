...
def test_claude_desktop_handshake_initialize_list_call(monkeypatch, tmp_path: Path) -> None:
    ...
    # MCP-safe published name (dots replaced); the dotted id stays internal.
    assert "fetch_world_bank" in tool_names
    assert "fetch.world_bank" not in tool_names
    ...
    assert parsed["output_preview"]["provider"] == "fetch.world_bank"
