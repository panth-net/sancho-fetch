...
def test_claude_update_skill_exists_with_required_pieces() -> None:
    ...
    for must_have in (
        ...
        "backup_id",
        "sancho repair note",
        # PyPI is the user path: the skill must branch on install_source and
        # know the exact upgrade command.
        "install_source",
        "uv tool upgrade sancho-fetch",
        # The nudge must actually reach the user.
        "update_hint",
    ):
...
def test_codex_update_skill_exists_with_required_pieces() -> None:
    ...
    for must_have in (
        ...
        "install_source",
        "uv tool upgrade sancho-fetch",
        # GitHub Desktop survives as the checkout/developer branch only.
        "GitHub Desktop",
    ):
