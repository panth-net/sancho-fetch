...
def test_default_workspace_config_enables_raw_cache(tmp_path: Path) -> None:
    from sancho.config import load_workspace_config
    from sancho.utils import write_yaml

    assert main(["init", "--path", str(tmp_path), "--yes"]) == 0
    ws = tmp_path / "sancho-workspace"
    cfg = load_workspace_config(ws)
    assert cfg["runtime"]["raw_cache"]["enabled"] is True
    assert cfg["runtime"]["raw_cache"]["max_age_seconds"] == 86400

    # A workspace whose sancho.yaml predates the key still gets it via the merge
    # backfill (so reuse turns on for existing installs, not just new ones).
    write_yaml(ws / "sancho.yaml", {"version": 1, "runtime": {"logging": {"format": "json"}}})
    cfg2 = load_workspace_config(ws)
    assert cfg2["runtime"]["raw_cache"]["enabled"] is True


def test_resolve_staleness_refresh_forces_fresh_fetch() -> None:
    runtime = {"raw_cache": {"enabled": True, "max_age_seconds": 86400}}
    # No refresh -> honor the configured window.
    assert resolve_staleness_seconds(payload={}, runtime=runtime, module_id="m") == 86400
    # refresh -> 0s window means nothing is fresh enough to reuse -> fetch.
    assert resolve_staleness_seconds(payload={"refresh": True}, runtime=runtime, module_id="m") == 0
    assert resolve_staleness_seconds(
        payload={"cache": {"refresh": True}}, runtime=runtime, module_id="m"
    ) == 0
...
def test_module_runtime_reuses_fresh_raw_cache_by_default(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
    module_id: str,
    input_payload: dict[str, Any],
    ...
    # Reuse is ON by default (DEFAULT_SANCHO_CONFIG runtime.raw_cache): an identical
    # request with a fresh cached record is served from cache (no API call), and
    ...
    # Default: the just-saved record is fresh -> reuse it, no network call.
    result = run_module(workspace, module_id=module_id, input_payload=input_payload)
    assert result.status == "ok"
    assert calls["count"] == 0
    assert result.output["raw"] == cached_raw

    # refresh:true forces a fresh fetch even though a fresh record exists.
    refreshed = run_module(
        workspace, module_id=module_id, input_payload={**input_payload, "refresh": True}
    )
    assert refreshed.status == "ok"
    assert calls["count"] == 1
    assert refreshed.output["raw"] == network_raw
