...
def _isolate_cwd(tmp_path_factory: pytest.TempPathFactory, monkeypatch: pytest.MonkeyPatch) -> None:
    """Every test runs in its own temp cwd.

    Auto public exports resolve their target from Path.cwd(); without this,
    any test that runs a fetch module writes mock .xlsx files into the real
    repo's sancho-downloads/ (hundreds accumulated before this fixture).
    """
