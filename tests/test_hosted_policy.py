...
def test_hosted_bootstrap_selection_covers_allowlist() -> None:
    """The hosted server boots the quick workspace with modules_csv set to the
    allowlist (hosting/server.py). That resolved selection must cover every
    allowlisted id -- a named profile alone once missed three modules and the
    server refused to boot."""
    from sancho.mcp.quick import resolve_quick_selection

    selection = resolve_quick_selection(
        profile="broad", modules_csv=",".join(sorted(HOSTED_PROVIDERS))
    )
    missing = sorted(HOSTED_PROVIDERS - set(selection.resolved_modules))
    assert not missing, f"hosted bootstrap would not install: {missing}"
