...
def test_export_writes_public_working_output_with_primary_path(tmp_path: Path) -> None:
    workspace = _init_workspace(tmp_path)
    _seed_record(workspace, {"format": "json"})
    fetched_root = workspace / "fetched-data"
    record_dirs = [d for d in fetched_root.rglob("data.json")]
    ...
    # Public output is the user's working file under sancho-downloads/, not a
    # bundle folder with manifests.
    assert result.primary_path.exists()
    assert result.export_root.name == "sancho-downloads"
    assert str(record_dir) in [str(p) for p in result.canonical_record_dirs]
