...
**If you are a person (not an AI):** you don't need to read this. Just tell
your AI assistant what data source you want (an API, an agency's data, a file
you receive regularly) and ask it to create a Sancho module for it. The AI
follows this guide.
...
## Goal (future work -- `sancho module create` does NOT exist yet)

Add a `sancho module create` scaffolder so new modules are created through one
safe path instead of hand-built ad hoc folders. Everything in this section is a
design proposal: the command is not implemented, so do not run or suggest it.
Use the manual workflow above.

Proposed command shape (not runnable today):
...
When your module downloads a real file (XLSX, parquet, ZIP, KML, ...), preserve
the original so the cache stays faithful and the public export can hand the user
the true source:

- If you fetch with `sancho.runtime.net.download_file(...)`, this is **automatic**
  -- the downloaded file is captured for you.
- If you read raw bytes yourself (e.g. `requests.get(url).content`), call
  `note_original_bytes(content, url, resp.headers.get("content-type"))` from
  `sancho.runtime.net` right after the download, or pass `original_bytes=...`,
  `original_filename=...`, `original_media_type=...` directly to `save_raw(...)`.

Still return clean tabular `rows` from your transform: a clean table becomes a
CSV in the public output, while the original file is preserved alongside (or as
the primary output when a CSV would lose information, e.g. multi-tab Excel or
geospatial formats).
