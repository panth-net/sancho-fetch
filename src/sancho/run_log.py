...
# The tail views (CLI `sancho log tail`, MCP sancho_log_tail) hide these
# fields: they repeat identically in every event and are a third of the bytes
# an agent reads after each fetch. `sancho log show <run-id>` keeps them.
TAIL_HIDDEN_FIELDS = ("workspace_path", "current_project_path", "module_path")
...
def tail_events_view(events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return [
        {k: v for k, v in event.items() if k not in TAIL_HIDDEN_FIELDS}
        for event in events
    ]
