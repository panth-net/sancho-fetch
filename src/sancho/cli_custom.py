...
from sancho.path_utils import is_hidden_relpath
...
def cmd_module_compare(args: argparse.Namespace) -> int:
    ...
    if custom and template:
        custom_files = {
            rel: p
            for p in custom.module_dir.rglob("*")
            if p.is_file() and not is_hidden_relpath(rel := p.relative_to(custom.module_dir).as_posix())
        }
        template_files = {
            rel: p
            for p in template.template_dir.rglob("*")
            if p.is_file() and not is_hidden_relpath(rel := p.relative_to(template.template_dir).as_posix())
        }
