...
def env_candidate_paths(workspace_root: Path) -> list[Path]:
    """Return the env files Sancho reads.

    Sancho keeps exactly one .env, inside the workspace, so API keys and
    `sancho mode` always land in the same file and no file shadows another.
    Still a list because callers report the paths they checked.
    """
    return [workspace_env_path(workspace_root)]


def load_env_values(workspace_root: Path) -> dict[str, str]:
    """Load Sancho env values from the workspace .env."""
    merged: dict[str, str] = {}
...
def resolve_env_edit_path(workspace_root: Path) -> Path:
    """Return the one .env Sancho reads and the user edits.

    `sancho env open` opens this for the user, so it does not need to sit
    somewhere an editor happens to show by default.
    """
    return workspace_env_path(workspace_root)


def env_status(workspace_root: Path) -> dict[str, Any]:
    """Names-only status for the Sancho env file.

    Reports enough to diagnose a missing key without exposing values.
    """
    ...
    for path in env_candidate_paths(workspace_root):
        by_path.append({
            "path": str(path),
            "exists": path.exists(),
            "keys_present": sorted(read_env_values(path)),
        })
    ...
    return {
        ...
        "workspace_env_path": str(workspace_env_path(workspace_root)),
        "env_paths": by_path,
        "keys_present": sorted(load_env_values(workspace_root)),
        "note": (
            "Sancho keeps exactly one .env, inside sancho-workspace/. Only "
            "env-var NAMES are reported. Values are never printed."
        ),
    }


def _env_example_contents(workspace_root: Path) -> tuple[Path, str | None]:
    """Find a .env.example to embed. Prefer the workspace's copy; fall back to the package's bundled template."""
    ...
    if BUNDLED_ENV_EXAMPLE.exists():
        try:
            return BUNDLED_ENV_EXAMPLE, BUNDLED_ENV_EXAMPLE.read_text(encoding="utf-8", errors="replace")
        except OSError:
            pass
    return workspace_copy, None


def _env_example_hints(text: str | None, keys: set[str]) -> list[str]:
    """Return only the `# --- <provider> ---` blocks that define one of `keys`.

    Embedding all of .env.example added ~16KB to every recommend payload; an
    agent only needs the sign-up hints for keys that are actually missing.
    """
    ...
    return [
        ...
        if any(line.startswith(prefixes) for line in section)
    ]
...
def env_recommend(workspace_root: Path, query: str, *, limit: int = 8) -> dict[str, Any]:
    ...
    status + the .env.example sections for the missing keys (sign-up URLs and
    per-key form-field hints). Values from .env are never read or returned.
    """
    ...
    candidates = find_sources(query, limit=limit, workspace_root=workspace_root)
    populated = set(status_payload["keys_present"])

    # Custom modules aren't in the static MODULE_KEYS map; their key needs
    # come from the `api_key_env` field of their own manifest (the same field
    # the runtime honors at fetch time).
    custom_manifest_keys: dict[str, list[str]] = {}
    if any(c.kind == "custom" for c in candidates):
        from sancho.modules import discover_modules
        from sancho.runtime.soft_validation import required_env_keys

        for module in discover_modules(workspace_root, zone="custom", strict=False):
            custom_manifest_keys[module.id] = required_env_keys(module.manifest)
    ...
    for c in candidates:
        ...
        for name in custom_manifest_keys.get(c.module_id, []):
    ...
    if missing_required:
        ...
        next_steps.append(
            "Sign-up URLs and step-by-step instructions for each missing key "
            "are embedded in this payload as `env_example_hints` (the relevant "
    ...
    return {
        ...
        **status_payload,
        ...
        "env_exists": env_path.exists(),
        "env_example_path": str(env_example_path),
        "env_example_hints": _env_example_hints(env_example_text, hint_keys),
        "candidates": rows,
        "summary": {
            "ready": ready_count,
