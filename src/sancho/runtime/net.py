...
def note_original_bytes(
    ...
    *,
    ...
    """Record a downloaded original artifact for the run.

    Modules that fetch a real file (xlsx/zip/parquet/...) call this right after
    the download so ``save_raw`` keeps the original in the cache byte-for-byte.
    Pass ``data`` for in-memory bytes, or ``path`` for a file already on disk
    (cheaper for large archives). No-ops on any failure.
    """
    try:
        from sancho.runtime import request_state as _request_state

        _request_state.note_pending_original(
    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            # Record the original artifact so the next save_raw keeps it in the
            # cache byte-for-byte (no per-module wiring needed).
            try:
                from sancho.runtime import request_state as _request_state

                _request_state.note_pending_original(
                    path=final_path, filename=final_name, media_type=ct,
                )
            except Exception:
                pass
