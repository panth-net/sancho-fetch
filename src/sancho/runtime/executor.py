...
def run_module(workspace_root: Path, module_id: str, input_payload: dict[str, Any] | None = None) -> ModuleRunResult:
    ...
    # Walk only this module's subtree: the before/after diff reports files this
    # run wrote, and a whole-cache walk grows with every other module's history.
    from sancho.templates.runtime.data_store import _sanitize_segment

    module_data_path = fetched_data_path / _sanitize_segment(module_id)
    files_before = _list_files(module_data_path)
    ...
    if soft_warnings:
        for warning in soft_warnings:
            print(f"[soft-validate] {warning}", file=sys.stderr)
    ...
    if missing_keys:
        from sancho.env_keys import env_candidate_paths, resolve_env_edit_path

        env_path = resolve_env_edit_path(workspace_root)
        candidates = [str(p) for p in env_candidate_paths(workspace_root)]
        if len(candidates) > 1:
            checked_note = (
                f"Sancho already checked {' and '.join(candidates)} "
                "(workspace overrides project), so the key is in neither. "
            )
        else:
            checked_note = ""
        message = (
            ...
            f"{checked_note}Add it to {env_path} (or run `sancho env open`) and re-run."
            f"{_api_key_hint(module_manifest, missing_keys)}"
        )
        packet = write_error_packet(
            run_handle,
            error_message=message,
            **_packet_kwargs(workspace_root, module_manifest, module_id, module_data_path, files_before),
        )
        finish_run(
            run_handle,
    ...
    try:
        ...
        packet = write_error_packet(
            ...
            **_packet_kwargs(workspace_root, module_manifest, module_id, module_data_path, files_before, exc=exc),
        )
        finish_run(
            run_handle,
            status="failed",
    ...
    # Surface which cache record(s) this run wrote/reused + cache hit/miss, so
    # callers don't guess "latest record for module".
    record_dir_strs, cache_status = request_state.summarize_run_records()
    record_dirs = [Path(p) for p in record_dir_strs]
    finish_run(
        run_handle,
    ...
    return ModuleRunResult(
        module_id=module_id, status="ok", output=output,
        record_dirs=record_dirs, cache_status=cache_status,
    )
