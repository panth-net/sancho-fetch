...
def _open_in_editor(path: Path) -> None:
    ...
    if system == "Windows" and hasattr(os, "startfile"):
        try:
            ...
            # Dotfiles like .env have no associated app; Notepad always exists.
            subprocess.run(["notepad", target], check=False)
        return
    if system == "Darwin":
        # Plain `open` fails when no app claims the file type (true for
        # dotfiles like .env) -- and it fails quietly, so the user is told
        # the file was opened when nothing appeared. `-t` forces the default
        # text editor; `-e` (TextEdit) is the always-present fallback.
        result = subprocess.run(["open", "-t", target], check=False)
        if result.returncode != 0:
            subprocess.run(["open", "-e", target], check=False)
        return
...
def cmd_env_open(args: argparse.Namespace) -> int:
    ...
    if not env_path.exists():
        ...
        if not env_example.exists() and BUNDLED_ENV_EXAMPLE.exists():
            # A missing workspace .env.example (deleted, or an old workspace)
            # is restored from the package's bundled template so the user
            # keeps the per-key sign-up instructions.
            shutil.copy2(BUNDLED_ENV_EXAMPLE, env_example)
            print(f"Restored {env_example} from the bundled template")
