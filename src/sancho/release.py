...
def _template_files(template_dir: Path) -> list[Path]:
    """All template files, sorted — excluding interpreter droppings, which
    appear inside installed templates once their modules are imported and
    would otherwise make manifests nondeterministic."""
    return sorted(
        p
        for p in template_dir.rglob("*")
        if p.is_file()
        and "__pycache__" not in p.relative_to(template_dir).parts
...
def _template_sha(template_dir: Path) -> str:
    ...
    h = hashlib.sha256()
    for path in _template_files(template_dir):
    ...
    for module_id, template in sorted(registry.items()):
        modules[module_id] = {
            ...
            "paths": [
                ...
                for path in _template_files(template.template_dir)
            ],
        }
    return {
