...
def resolve_staleness_seconds(
    *,
    ...
    # Explicit refresh forces a fresh fetch (0s window -> nothing is fresh enough
    # to reuse). Honor it before any configured window.
    if _parse_bool(payload.get("refresh")) or _parse_bool(payload.get("force_refresh")) or (
        isinstance(payload_cache, dict) and _parse_bool(payload_cache.get("refresh"))
    ):
        return 0
