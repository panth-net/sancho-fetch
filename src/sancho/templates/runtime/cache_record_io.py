...
def save_record(
    *,
    ...
    When ``original_bytes`` is provided the source's original artifact is stored
    byte-for-byte as ``original.<ext>`` alongside the parsed ``data.json``, and
    the provenance gains ``original_*`` fields with ``source_kind: file``.
    """
    counter = 1
    base_dir = record_dir
    ...
    # Optionally persist the original downloaded file, untouched.
    ...
    original_sha256 = ""
    if isinstance(original_bytes, (bytes, bytearray)) and len(original_bytes) > 0:
        from sancho.templates.runtime.file_formats import detect_extension
        ...
        original_path = record_dir / original_name
        original_path.write_bytes(bytes(original_bytes))
        original_sha256 = hashlib.sha256(bytes(original_bytes)).hexdigest()
        original_fields = {
            "source_kind": "file",
            "original_file": original_name,
            "original_media_type": str(original_media_type or "").split(";", 1)[0].strip(),
    ...
    hash_lines = [f"sha256:{content_sha256}  {DATA_FILE}"]
    if original_sha256:
        hash_lines.append(f"sha256:{original_sha256}  {original_fields['original_file']}")
