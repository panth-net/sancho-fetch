...
from sancho.templates.runtime.cache_index import PROVENANCE_FILE
...
from sancho.templates.runtime.file_formats import resolve_pending_original
...
# Cache path segments are bounded so a long family/module id can't create an
# over-long directory name. Short values (the common case) are returned
# byte-for-byte identical to the historical scheme; only segments longer than
# this cap are truncated + hashed. Kept conservative (40) because Windows caps
# the *total* path at 260 and two stacked segments + the workspace prefix +
# timestamp eat the budget fast.
_MAX_SEGMENT_LEN = 40
...
def _sanitize_segment(value: str) -> str:
    ...
    if len(safe) > _MAX_SEGMENT_LEN:
        ...
        keep = max(1, _MAX_SEGMENT_LEN - 9)  # room for "_" + 8-char hash
        safe = f"{safe[:keep].strip('._') or 'seg'}_{digest}"
    return safe


def _to_filename_timestamp(moment: datetime) -> str:
    # Second resolution (no microseconds) keeps the segment short; same-second
    # collisions for one request_key are handled by save_record's counter.
...
def _save_raw_record(
    *,
    ...
    # Auto-attach an original artifact recorded earlier this run, keeping the
    # cache faithful with no per-module save_raw wiring.
    original_bytes, original_filename, original_media_type = resolve_pending_original(
        original_bytes, original_filename, original_media_type
    )
    ...
    # Surface the written record to the executor (Phase 5).
    try:
        from sancho.runtime import request_state as _request_state
        _request_state.note_record_saved(record_dir)
    except Exception:
        pass
...
def _load_raw_record(
    *,
    ...
    if loaded is None:
        return None
    # Surface the cache hit to the executor (Phase 5).
    try:
        from sancho.runtime import request_state as _request_state
        _request_state.note_record_reused(loaded["record_dir"])
    except Exception:
        pass
    return RawCacheRecord(
        raw=loaded["raw"],
