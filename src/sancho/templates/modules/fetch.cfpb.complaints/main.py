...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_cfpb_complaints_api")
    transform = _load_sibling("transform.py", "sancho_fetch_cfpb_complaints_transform")

    endpoint = str(payload.get("endpoint", "https://www.consumerfinance.gov/data-research/consumer-complaints/search/api/v1/"))
