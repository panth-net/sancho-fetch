...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_ahrq_meps_api")
    transform = _load_sibling("transform.py", "sancho_fetch_ahrq_meps_transform")

    puf_id = str(payload.get("puf_id", "h233")).lower()
    limit_obj = payload.get("limit", 1000)
    limit = limit_obj if isinstance(limit_obj, int) and limit_obj > 0 else 1000

    source_url = api.build_source_url(puf_id=puf_id)
    family_or_dataset_id = f"meps_{puf_id}"
    params = {"puf_id": puf_id, "limit": limit}
