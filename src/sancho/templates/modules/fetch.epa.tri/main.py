...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_epa_tri_api")
    transform = _load_sibling("transform.py", "sancho_fetch_epa_tri_transform")

    endpoint = str(
        payload.get(
            "endpoint",
            "https://data.epa.gov/efservice/tri_facility/state_abbr/CA/Rows/0:25/JSON",
        )
    )
    ...
    if cached is not None:
        return transform.build_output(
            endpoint=endpoint, raw=cached.raw, params=params,
        )
    ...
    return transform.build_output(
        endpoint=endpoint, raw=raw, params=params,
    )
