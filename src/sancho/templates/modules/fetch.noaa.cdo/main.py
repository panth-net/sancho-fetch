...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_noaa_cdo_api")
    transform = _load_sibling("transform.py", "sancho_fetch_noaa_cdo_transform")

    endpoint = str(payload.get("endpoint", "https://www.ncei.noaa.gov/cdo-web/api/v2/datasets"))
    ...
    raw = api.fetch_dataset(
        ...
        api_token=context.env.get("NOAA_API_TOKEN", ""),
        endpoint=endpoint,
        params=params,
    )
    save_raw(
