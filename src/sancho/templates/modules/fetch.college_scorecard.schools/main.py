...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_college_scorecard_schools_api")
    transform = _load_sibling("transform.py", "sancho_fetch_college_scorecard_schools_transform")

    endpoint = str(payload.get("endpoint", "https://api.data.gov/ed/collegescorecard/v1/schools.json"))
