...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_wvs_api")
    transform = _load_sibling("transform.py", "sancho_fetch_wvs_transform")

    file_path_raw = payload.get("file_path")
    file_path = str(file_path_raw) if isinstance(file_path_raw, str) and file_path_raw.strip() else None
    country_raw = payload.get("country")
    country = str(country_raw).upper() if isinstance(country_raw, str) and country_raw.strip() else None
    wave = payload.get("wave") if isinstance(payload.get("wave"), int) else None

    source_url = api.build_source_url()
    params = {"file_path": file_path, "country": country, "wave": wave}
    family_or_dataset_id = f"wvs_{country or 'all'}_{wave or 'all'}"
    ...
    # Fetch (or return manual-download instructions)
    raw = api.fetch_wvs(
