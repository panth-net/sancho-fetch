...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_owid_catalog_api")
    transform = _load_sibling("transform.py", "sancho_fetch_owid_catalog_transform")

    # Extract payload fields with defaults
    search = payload.get("search")
    search_filter = (
        str(search).strip() if isinstance(search, str) and search.strip() else None
    )
    tag = payload.get("tag")
    tag_filter = (
        str(tag).strip() if isinstance(tag, str) and tag.strip() else None
    )
    limit = payload.get("limit")
    limit_val = int(limit) if isinstance(limit, int) and limit > 0 else 500

    # Build request metadata
    source_url = api.CHARTS_URL
    params = {"search": search_filter, "tag": tag_filter, "limit": limit_val}
    family_or_dataset_id = "owid_catalog"
