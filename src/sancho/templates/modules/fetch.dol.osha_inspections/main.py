...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_dol_osha_inspections_api")
    transform = _load_sibling("transform.py", "sancho_fetch_dol_osha_inspections_transform")

    endpoint = str(payload.get("endpoint", "https://apiprod.dol.gov/v4/get/OSHA/inspection/json"))
