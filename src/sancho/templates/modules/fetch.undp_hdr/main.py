...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_undp_hdr_api")
    transform = _load_sibling("transform.py", "sancho_fetch_undp_hdr_transform")
    ...
    indicator = str(payload.get("indicator", "")) if payload.get("indicator") else None

    source_url = api.build_source_url(year=year)
    params: dict[str, Any] = {"year": year, "country": country_filter, "indicator": indicator}
    family_or_dataset_id = f"undp_hdr_{year or 'latest'}"
    ...
    if max_age_seconds is not None or cache_enabled:
        ...
        cached = load_raw(
            data_raw_path=context.data_raw_path, module_id=MODULE_ID,
            family_or_dataset_id=family_or_dataset_id, params=params,
            source_url=source_url, max_age_seconds=effective,
        )
    ...
    save_raw(
        data_raw_path=context.data_raw_path, module_id=MODULE_ID,
        family_or_dataset_id=family_or_dataset_id, raw=raw,
        params=params, source_url=source_url,
    )
