...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_noaa_nws_api")
    transform = _load_sibling("transform.py", "sancho_fetch_noaa_nws_transform")

    endpoint = str(
        payload.get("endpoint", "https://api.weather.gov/alerts/active")
    )
