...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_atsdr_svi_api")
    transform = _load_sibling("transform.py", "sancho_fetch_atsdr_svi_transform")

    year_obj = payload.get("year", 2022)
    year = year_obj if isinstance(year_obj, int) else 2022
    state_obj = payload.get("state")
    state = (
        str(state_obj).strip().upper()
    ...
    family_or_dataset_id = f"svi_{year}_{state or 'all'}"
    params = {"year": year, "state": state, "limit": limit}
