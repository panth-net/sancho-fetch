...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_fdic_institutions_api")
    transform = _load_sibling("transform.py", "sancho_fetch_fdic_institutions_transform")

    endpoint = str(payload.get("endpoint", "https://banks.data.fdic.gov/api/institutions"))
