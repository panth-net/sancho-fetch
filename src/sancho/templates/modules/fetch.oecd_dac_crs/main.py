...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_oecd_dac_crs_api")
    transform = _load_sibling("transform.py", "sancho_fetch_oecd_dac_crs_transform")

    # Extract payload fields with defaults
    dataflow = payload.get("dataflow")
    dataflow_str = (
        str(dataflow).upper()
        if isinstance(dataflow, str) and dataflow.strip()
        else "CRS"
    )

    # Build request metadata
    source_url = api.build_source_url(dataflow=dataflow_str)
    params = {"dataflow": dataflow_str}
    family_or_dataset_id = f"oecd_dac_crs_{dataflow_str.lower()}"
