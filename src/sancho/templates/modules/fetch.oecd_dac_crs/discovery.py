"""Live catalog discovery for OECD DAC/CRS SDMX. Uses the shared SDMX helper."""
...
def _load_blueprint() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("sancho_oecd_dac_blueprint", path)
...
def discover(*, module_dir: Path, offline: bool = False) -> dict[str, Any]:
    return discover_sdmx(
        ...
        provider_id=str(getattr(BLUEPRINT, "PROVIDER_ID", "fetch.oecd_dac_crs")),
        base_url=str(getattr(BLUEPRINT, "BASE_URL", "https://sdmx.oecd.org/dcd-public/rest")),
