"""Live catalog discovery for openpaymentsdata.cms.gov (DCAT /data.json)."""
...
def _load_blueprint() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("sancho_open_payments_blueprint", path)
...
def discover(*, module_dir: Path, offline: bool = False) -> dict[str, Any]:
    return discover_dcat(
        ...
        provider_id=str(getattr(BLUEPRINT, "PROVIDER_ID", "fetch.open_payments.datasets")),
        base_url=str(getattr(BLUEPRINT, "BASE_URL", "https://openpaymentsdata.cms.gov")),
