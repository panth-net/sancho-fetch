...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_eia_series_api")
    transform = _load_sibling("transform.py", "sancho_fetch_eia_series_transform")

    endpoint = str(payload.get("endpoint", "https://api.eia.gov/v2/seriesid/PET.RWTC.D"))
    ...
    raw = api.fetch_dataset(
        ...
        api_token=context.env.get("EIA_API_KEY", ""),
        endpoint=endpoint,
        params=params,
    )
    save_raw(
