...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_airnow_api")
    transform = _load_sibling("transform.py", "sancho_fetch_airnow_transform")

    # Default: current observation by ZIP code (New York / 10001) within 25 miles.
    endpoint = str(
        payload.get(
            "endpoint",
    ...
    params = {"zipCode": "10001", "distance": 25, **params}
