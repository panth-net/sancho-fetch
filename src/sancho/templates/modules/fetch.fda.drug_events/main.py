...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_fda_drug_events_api")
    transform = _load_sibling("transform.py", "sancho_fetch_fda_drug_events_transform")

    endpoint = str(payload.get("endpoint", "https://api.fda.gov/drug/event.json"))
