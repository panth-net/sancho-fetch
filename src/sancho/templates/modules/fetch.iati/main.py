...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_iati_api")
    transform = _load_sibling("transform.py", "sancho_fetch_iati_transform")

    # Extract payload fields with defaults
    reporting_org_raw = payload.get("reporting_org")
    reporting_org: str | None = (
        str(reporting_org_raw).strip()
        if isinstance(reporting_org_raw, str) and reporting_org_raw.strip()
        else None
    )
    limit_raw = payload.get("limit")
    limit: int = int(limit_raw) if isinstance(limit_raw, int) and limit_raw > 0 else 1000

    # Build request metadata
    source_url = api.build_source_url()
    params = {"reporting_org": reporting_org, "limit": limit}
    family_or_dataset_id = f"iati_{reporting_org or 'all'}"
