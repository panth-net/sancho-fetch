...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_treasury_fiscal_data_api")
    transform = _load_sibling("transform.py", "sancho_fetch_treasury_fiscal_data_transform")

    endpoint = str(payload.get("endpoint", "https://api.fiscaldata.treasury.gov/services/api/fiscal_service/v1/accounting/od/rates_of_exchange"))
