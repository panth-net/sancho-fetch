...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_usgs_earthquakes_api")
    transform = _load_sibling("transform.py", "sancho_fetch_usgs_earthquakes_transform")

    endpoint = str(payload.get("endpoint", "https://earthquake.usgs.gov/fdsnws/event/1/query"))
