...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_usaspending_awards_api")
    transform = _load_sibling("transform.py", "sancho_fetch_usaspending_awards_transform")

    endpoint = str(payload.get("endpoint", "https://api.usaspending.gov/api/v2/search/spending_by_award/"))
