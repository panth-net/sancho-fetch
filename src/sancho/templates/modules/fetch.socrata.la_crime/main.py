...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_socrata_la_crime_api")
    transform = _load_sibling("transform.py", "sancho_fetch_socrata_la_crime_transform")

    endpoint = str(payload.get("endpoint", "https://data.lacity.org/resource/2nrs-mtv8.json"))
