...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_oecd_sdmx_api")
    transform = _load_sibling("transform.py", "sancho_fetch_oecd_sdmx_transform")

    # Extract payload fields with defaults
    dataset = payload.get("dataset")
    dataset = str(dataset).strip() if isinstance(dataset, str) and dataset.strip() else "current_wellbeing"
    ...
    source_url = api.build_source_url(dataset=dataset)
    params = {"dataset": dataset, "country": country_filter}
    family_or_dataset_id = f"oecd_sdmx_{dataset}"
