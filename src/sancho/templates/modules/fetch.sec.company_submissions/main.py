...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_sec_company_submissions_api")
    transform = _load_sibling("transform.py", "sancho_fetch_sec_company_submissions_transform")

    endpoint = str(payload.get("endpoint", "https://data.sec.gov/submissions/CIK0000320193.json"))
