...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_fbi_crime_api")
    transform = _load_sibling("transform.py", "sancho_fetch_fbi_crime_transform")

    endpoint = str(payload.get("endpoint", "https://api.usa.gov/crime/fbi/cde/arrest/state/CA/all"))
    params_obj = payload.get("params", {"type": "counts", "from": "01-2020", "to": "12-2022"})
    ...
    raw = api.fetch_dataset(
        ...
        api_token=context.env.get("DATA_GOV_API_KEY", ""),
        endpoint=endpoint,
        params=params,
    )
    save_raw(
