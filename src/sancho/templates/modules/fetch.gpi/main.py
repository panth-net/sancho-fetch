...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_gpi_api")
    transform = _load_sibling("transform.py", "sancho_fetch_gpi_transform")
    ...
    family_or_dataset_id = f"gpi_{year or 'latest'}"
    ...
    raw = api.fetch_gpi(
        ...
        dest_dir=context.data_raw_path / "fetch_gpi",
        year=year,
