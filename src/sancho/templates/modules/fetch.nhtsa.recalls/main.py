...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_nhtsa_recalls_api")
    transform = _load_sibling("transform.py", "sancho_fetch_nhtsa_recalls_transform")

    endpoint = str(payload.get("endpoint", "https://api.nhtsa.gov/recalls/recallsByVehicle"))
    params_obj = payload.get("params", {"make": "Honda", "model": "Civic", "modelYear": 2024})
