...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_usda_fooddata_search_api")
    transform = _load_sibling("transform.py", "sancho_fetch_usda_fooddata_search_transform")

    endpoint = str(payload.get("endpoint", "https://api.nal.usda.gov/fdc/v1/foods/search"))
