...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_census_api")
    transform = _load_sibling("transform.py", "sancho_fetch_census_transform")
    ...
    year = str(payload.get("year", "2021"))
    variables_obj = payload.get("variables", ["NAME", "DP05_0001E"])
    variables = variables_obj if isinstance(variables_obj, list) else ["NAME", "DP05_0001E"]

    source_url = f"https://api.census.gov/data/{year}/acs/acs5/profile"
    family_or_dataset_id = geography.strip() if isinstance(geography, str) and geography.strip() else "default"
    if in_geography:
        family_or_dataset_id = f"{family_or_dataset_id}__in__{in_geography.replace(':', '_')}"
    params = {
        "geography": geography,
    ...
    if cached is not None:
        return transform.build_output(geography=geography, year=year, rows=cached.raw)

    rows = api.fetch_acs_profile_rows(
    ...
    return transform.build_output(geography=geography, year=year, rows=rows)
