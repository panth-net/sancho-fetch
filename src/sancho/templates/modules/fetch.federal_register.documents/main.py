...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_federal_register_documents_api")
    transform = _load_sibling("transform.py", "sancho_fetch_federal_register_documents_transform")

    endpoint = str(payload.get("endpoint", "https://www.federalregister.gov/api/v1/documents.json"))
