...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_socrata_sf_building_permits_api")
    transform = _load_sibling("transform.py", "sancho_fetch_socrata_sf_building_permits_transform")

    endpoint = str(payload.get("endpoint", "https://data.sfgov.org/resource/i98e-djp9.json"))
