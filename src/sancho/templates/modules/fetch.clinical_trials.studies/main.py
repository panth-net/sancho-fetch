...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_clinical_trials_studies_api")
    transform = _load_sibling("transform.py", "sancho_fetch_clinical_trials_studies_transform")

    endpoint = str(payload.get("endpoint", "https://clinicaltrials.gov/api/v2/studies"))
