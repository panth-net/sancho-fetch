...
def run(*, context: ModuleContext, payload: dict[str, Any]) -> dict[str, Any]:
    api = _load_sibling("api.py", "sancho_fetch_pew_api")
    transform = _load_sibling("transform.py", "sancho_fetch_pew_transform")
    ...
    page_val = int(page) if isinstance(page, int) and page > 0 else 1
    limit = payload.get("limit")
    limit_val = int(limit) if isinstance(limit, int) and limit > 0 else 20

    # Build request metadata
    source_url = api.build_source_url(page=page_val)
    params = {"search": search_filter, "page": page_val, "limit": limit_val}
    family_or_dataset_id = "pew_research"
