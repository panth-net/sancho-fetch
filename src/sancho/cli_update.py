...
from sancho.self_update import package_status
...
def cmd_update_check(args: argparse.Namespace) -> int:
    ...
    # The explicit check is the one moment a live PyPI probe is warranted;
    # ambient callers (ready, paths) stay on the stamped value.
    payload["package"] = package_status(workspace_root, probe=True)
    if getattr(args, "json", False):
        print(json.dumps(payload, indent=2, default=str))
        return 0
    print(f"# Update check (sancho {payload['sancho_version']})")
    package = payload["package"]
    if package["reinstall_needed"]:
        if package["install_source"] == "package":
            print(
                f"- NOTE: sancho-fetch {package['upstream_version']} is on PyPI but the"
                f" installed sancho command is {package['installed_version']}."
            )
            print(f"  Upgrade first: {package['reinstall_command']}")
        else:
            print(
                f"- NOTE: this folder is version {package['checkout_version']} but the installed"
                f" sancho command is {package['installed_version']}."
            )
            print(f"  Reinstall first: {package['reinstall_command']}")
        print("  Then run `sancho update check` again to see the new module versions.")
