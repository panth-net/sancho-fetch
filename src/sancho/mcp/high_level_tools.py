...
def build_high_level_tools(ctx: MCPContext) -> list[ToolSpec]:
    ...
    return [
        ...
        ToolSpec(
            ...
            description=(
                "List built-in starter packs and fetch providers, plus the workspace's "
                "custom modules. This is the full catalog and it is large -- when you "
                "already know what data is wanted, sancho_find_sources returns just "
        ...
        ToolSpec(
            ...
            description=(
                "Rank module candidates for a natural-language data request. "
                "The cheap first step for choosing a source."
            ),
            input_schema={
                "type": "object",
                "required": ["query"],
        ...
        ToolSpec(
            ...
            description=(
                "Report cache status for a module or a specific request. Optional "
                "inspection only -- fetch tools reuse the cache automatically, so "
                "there is no need to check before running."
            ),
            input_schema={
                "type": "object",
                "required": ["module_id"],
        ...
        ToolSpec(
            ...
            description=(
                "Execute one fetch module with the given input payload. Returns a "
                "run summary with a truncated preview; the full dataset is written "
                "to the file at primary_output_path."
            ),
            input_schema={
                "type": "object",
                "required": ["module_id"],
        ...
        ToolSpec(
            ...
            description="Copy a cached record into a project's 'sancho-downloads/' folder.",
            input_schema={
                "type": "object",
        ...
        ToolSpec(
            ...
            description=(
                "Tail recent run/error events from logs/runs.jsonl (or errors.jsonl). "
                "Useful after a playbook/multi-unit run or a failure; a single run's "
                "summary already contains its own evidence."
            ),
            input_schema={
                "type": "object",
                "properties": {
        ...
        ToolSpec(
            ...
            description=(
                "Show every event plus the full repair packet for one run_id -- "
                "the deep-dive after a failed run."
            ),
            input_schema={
                "type": "object",
                "required": ["run_id"],
