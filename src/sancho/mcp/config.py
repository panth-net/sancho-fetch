...
def _sancho_command() -> str:
    """Return a command MCP clients can launch without inheriting shell PATH.

    Paths inside a repo ``.venv`` are avoided: they break the moment the
    folder moves. The user-level uv tool shim (``~/.local/bin``) survives.
    """
    current = Path(sys.argv[0])
    if current.exists() and current.name.lower().startswith("sancho") and not _in_venv(current):
        return str(current.resolve())
    exe = "sancho.exe" if sys.platform == "win32" else "sancho"
    stable = Path.home() / ".local" / "bin" / exe
    if stable.exists():
        return str(stable)
    resolved = shutil.which("sancho")
    if resolved:
        return str(Path(resolved).resolve())
    if current.exists() and current.name.lower().startswith("sancho"):
        return str(current.resolve())
    return "sancho"
...
def generate_client_config(
    ...
    *,
    ...
    if client == "chatgpt-desktop" and "command" in server_def:
        ...
        # out exactly what to put in each field.
        payload["gui_setup"] = {
            "where": "ChatGPT desktop app -> Settings -> MCP servers -> Add server",
            "name": "sancho",
            "type": "STDIO",
            "command": " ".join(
                f'"{part}"' if " " in part else part
                for part in [server_def["command"], *server_def["args"]]
            ),
        }
    return payload
...
def _current_platform() -> str:
    # Seam for tests: patch this instead of sys.platform, which is shared
    # process-wide and breaks shutil.which on non-Windows Pythons.
    return sys.platform
