...
from sancho.mcp.high_level_handlers import run_result_summary
...
# MCP / Anthropic require tool names to match this pattern. Internal IDs use
# dots as namespace separators (e.g. "fetch.census.acs_profile") and family
# aliases can exceed 64 chars, neither of which is allowed on the wire, so every
# published tool name is normalized through these helpers at the MCP boundary.
# The dotted/long IDs stay intact internally for execution and CLI use.
...
def mcp_tool_name(raw: str) -> str:
    ...
    Replace every char outside ``[A-Za-z0-9_-]`` with ``_`` and cap at 64
    characters. The substitution runs before truncation, so the result is
    always char-safe; an empty/all-illegal input falls back to ``"tool"`` so the
    length is always in ``[1, 64]``. Mirrors the on-disk ``slugify_module_id``
    convention (``fetch.world_bank`` -> ``fetch_world_bank``).
    """
    return _MCP_NAME_UNSAFE_RE.sub("_", raw)[:64] or "tool"


def unique_tool_name(candidate: str, used: set[str]) -> str:
    """Return ``candidate`` made unique within ``used``, staying <=64 chars.

    ``candidate`` is assumed already char-safe and <=64 (i.e. produced by
    :func:`mcp_tool_name` or a hardcoded safe name). On collision a ``_N``
    suffix is carved *out of* the 64-char budget by trimming the base, so the
    result never exceeds 64 characters.
    """
...
def _build_gov_catalog_payload(
    ...
    # Map each module id to the exact tool name it is *published* under in
    # tools/list. Read the resolved (post de-dup) name from the registry rather
    # than recomputing mcp_tool_name(module_id): when two ids normalize to the
    # same value one is suffixed, and the catalog must advertise the real
    # callable name. Fall back to a fresh normalization only for ids missing
    # from the map (should not happen for installed modules).
    resolved_tool_names = {
        module_id: module_tool_names.get(module_id, mcp_tool_name(module_id))
    ...
    return {
        ...
        "module_tool_names": resolved_tool_names,
        "providers": providers,
...
class _ModuleToolHandler:
    ...
    def __call__(self, arguments: dict[str, Any]) -> Any:
        ...
        return run_result_summary(self.workspace_root, self.module_id, result)
...
class _GovCatalogToolHandler:
    ...
    modules: list[ModuleLocation]
    aliases: list[FamilyAliasBinding]
    # Shared, mutable map of module id -> final published tool name. Populated by
    # _tool_inventory after this spec is built (modules register after the
    # reserved core tools), and read here at call time when it is complete.
    module_tool_names: dict[str, str]

    def __call__(self, arguments: dict[str, Any]) -> Any:
        _ = arguments
        return _build_gov_catalog_payload(self.ctx, self.modules, self.aliases, self.module_tool_names)
...
class _FamilyAliasToolHandler:
    ...
    def __call__(self, arguments: dict[str, Any]) -> Any:
        ...
        return run_result_summary(self.workspace_root, self.binding.module_id, result)


def module_tool_spec(ctx: MCPContext, module: ModuleLocation) -> ToolSpec:
    return ToolSpec(
        name=mcp_tool_name(module.id),
...
def gov_fetch_tool_spec(ctx: MCPContext, modules: list[ModuleLocation]) -> ToolSpec:
    return ToolSpec(
        name="gov_fetch",
        description=(
            "Run a fetch provider by provider name or module ID using the provider "
            "payload contract. Returns a run summary with a truncated preview; the "
            "full dataset is written to the file at primary_output_path."
        ),
        input_schema={
            "type": "object",
            "required": ["provider"],
