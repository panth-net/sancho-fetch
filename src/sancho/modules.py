...
_template_registry_cache: dict[Path, dict[str, TemplateModule]] = {}


def load_template_registry() -> dict[str, TemplateModule]:
    # Bundled templates are immutable for the life of a process, and the
    # install path calls this three times per cold run -- memoize per root.
    cached = _template_registry_cache.get(MODULE_TEMPLATES_ROOT)
    ...
    _template_registry_cache[MODULE_TEMPLATES_ROOT] = registry
    return registry
...
def _copy_template(
    ...
    for src in template_dir.rglob("*"):
        ...
        if "__pycache__" in rel.parts or src.suffix == ".pyc":
            continue
        if protected and normalize_rel(rel) in protected:
            continue
        dst = target_dir / rel
...
def locally_edited_module_files(
    workspace_root: Path, module_id: str, target_dir: Path
) -> set[str]:
    """Module-relative paths whose contents no longer match modules.lock.yaml.

    The lock records what Sancho itself installed, so a checksum mismatch means
    a human or an agent edited a managed file. Reinstalling over it — which
    `sancho setup` and `sancho add` both do — would destroy that work silently.
    """
    lock = load_lock_config(workspace_root)
    entry = lock.get("modules", {}).get(module_id) or {}
    edited: set[str] = set()
    for rel_ws, expected in (entry.get("checksums") or {}).items():
        candidate = workspace_root / rel_ws
        if not candidate.exists() or file_sha256(candidate) == expected:
            continue
        try:
            edited.add(normalize_rel(candidate.relative_to(target_dir)))
        except ValueError:
            continue
    return edited
...
def _module_paths_from_manifest(module_dir: Path, manifest: dict[str, Any], workspace_root: Path) -> list[str]:
    ...
    if not managed:
        # Skip OS metadata (.DS_Store etc.) a file manager may have dropped in;
        # it must never become a "managed" file the update engine checksums.
        managed = [
            normalize_rel(path.relative_to(module_dir))
            for path in module_dir.rglob("*")
            if path.is_file() and not path.name.startswith(".") and path.name.lower() not in ("thumbs.db", "desktop.ini")
        ]
    result: list[str] = []
    for item in managed:
...
def discover_modules(workspace_root: Path, zone: str | None = None, *, strict: bool = True) -> list[ModuleLocation]:
    """Discover workspace modules.

    ``strict=False`` skips modules whose ``module.yaml`` is malformed instead
    of raising -- for search/list commands (find sources, inventory) that must
    keep working when one hand-edited custom module is broken. Diagnosing the
    broken module stays the job of ``sancho custom status`` / ``sancho run``,
    which use the strict default.
    """
...
def install_module(
    ...
    *,
    ...
    # Reinstalling must not silently destroy a managed file the user edited.
    # Same contract as `sancho update apply`, same opt-out flag.
    protected = (
        set()
        if allow_local_edits
        else locally_edited_module_files(workspace_root, module_id, target_dir)
    )
    _copy_template(template.template_dir, target_dir, overwrite=True, protected=protected)
...
def regenerate_lock(workspace_root: Path) -> dict[str, Any]:
    ...
    # Root-doc checksums are not module state, but they live here so there is a
    # single record of "what Sancho wrote". Carry them across regeneration.
    root_docs = previous_lock.get("root_docs")
    if root_docs:
        lock_payload["root_docs"] = root_docs
...
def resolve_module_for_execution(workspace_root: Path, module_id: str) -> ModuleLocation:
    ...
    # Auto-install from the bundled template registry on first use so callers
    # never have to `sancho add` before running. This is a fast local file copy
    # (discover=False -> no network); each module's catalog ships bundled with it.
    if module_id in load_template_registry():
        print(f"[auto-install] installing '{module_id}' from bundled templates", file=sys.stderr)
        install_target(workspace_root, module_id, discover=False)
        source = discover_module_map(workspace_root, zone="source")
        if module_id in source:
            return source[module_id]
    raise KeyError(
        f"Module '{module_id}' not found in custom or source, and is not a known "
        "bundled template. Run `sancho inventory --json` to see available modules."
    )
