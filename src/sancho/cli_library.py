...
def _paths_payload() -> dict[str, Any]:
    ...
    if workspace is not None:
        ...
        if git is not None:
            payload["git"] = git
        # The skills run `sancho paths` every session, so this is where the
        # throttled update nudge actually reaches the user (ready alone would
        # never be seen). At most one probe per 14 days; None otherwise.
...
def cmd_paths(args: argparse.Namespace) -> int:
    ...
    if "update_hint" in payload:
        print()
        print(f"  {payload['update_hint']}")
    return 0
